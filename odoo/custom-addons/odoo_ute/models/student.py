...
class Student(models.Model):
    ...
    state = fields.Selection(
        selection=[
            ...
            ('graduated', 'Graduado'),
            ('withdrawn', 'Retirado'),
            ('suspended', 'Suspendido'),],string='Estado', default='enrolled')
    
    campus = fields.Selection(
        selection=[
            ('quito', 'Quito - Sede Principal'),
            ('guayaquil', 'Guayaquil'),
            ('santo_domingo', 'Santo Domingo'),
            ('manta', 'Manta'),
            ('machala', 'Machala'),],string='Sede / Matriz', default='quito')
        ...
        if self.career_id:
            return {
                'warning': {
                    ...
                    'message': f'Se asigno la carrera: {self.career_id.name}'
                }
            }
        ...
        if self.gpa and (self.gpa < 0 or self.gpa > 10):
            return {
                'warning': {
                    ...
                    'message': 'Promedio debe estar entre 0 y 10.'
                }
            }

#@api.constrains oara validar el promedio
    @api.constrains('gpa')
    def _check_gpa(self):
        for rec in self:
            if rec.gpa < 0 or rec.gpa > 10:
                raise ValidationError("Promedio debe estar entre 0 y 10.")

    @api.constrains('vat')
    def _check_vat(self):
        for rec in self:
            if rec.vat and len(rec.vat) < 10:
                raise ValidationError("La CI debe tener 10 caracteres.")